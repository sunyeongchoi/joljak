...
<html>
<head><title>musical output</title></head>
<style>
...
<%
	...
	//Class.forName("oracle.jdbc.driver.OracleDriver");
      ...
      String midhigh = request.getParameter("midhigh");
      String college = request.getParameter("college");
      String sordier = request.getParameter("sordier");
      ...
      try
      {
            ...
            conn = DriverManager.getConnection(jdbcDriver, dbUser, dbPass);
         	// 3. Statement
            if(check.equals("again")){
                pstmt = conn.prepareStatement("select * from tickets_db.tickets where p_name like '%재관람%'");
            }else if(check.equals("midhigh")){
            	pstmt = conn.prepareStatement("select * from tickets_db.tickets where p_name like '%청소년%'");
            }else if(check.equals("college")){
            	pstmt = conn.prepareStatement("select * from tickets_db.tickets where p_name like '%대학%'");
            }else if(check.equals("sordier")){
            	pstmt = conn.prepareStatement("select * from tickets_db.tickets where p_name like '%군인%'");
            }else if(check.equals("multiple")){
            	pstmt = conn.prepareStatement("select * from tickets_db.tickets where p_name like '%다둥%'");
            }else if(check.equals("preview")){
            	pstmt = conn.prepareStatement("select * from tickets_db.tickets where p_name like '%프리뷰%'");
            }else if(check.equals("national")){
            	pstmt = conn.prepareStatement("select * from tickets_db.tickets where p_name like '%국가유공자%'");
            }else if(check.equals("disabled")){
            	pstmt = conn.prepareStatement("select * from tickets_db.tickets where p_name like '%장애%'");
            }
